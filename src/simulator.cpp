...
void Simulator::print_results(int N, vector<vector<string>> hands_str, vector<vector<int>> results)
{
    vector<vector<int>> hands = convert_multiple_hands(hands_str);
...
void Simulator::print_play(int N, vector<vector<string>> known_hands_str, vector<int> comm_hand, vector<vector<int>> results)
{
    if (!comm_hand.empty())
    {
        cout << endl;
        cout << "Community Hand:" << endl;
        print_hand(comm_hand);
        cout << endl;
        cout << endl;
    }
    cout << "Hand   Win %   Tie %" << endl;
    print_results(N, known_hands_str, results);
}
...
vector<vector<int>> Simulator::compute_probabilities(int N, vector<string> comm_hand_str, vector<vector<string>> known_hands_str, int players_unknown)
{
    ...
    print_play(N, known_hands_str, comm_hand, results);
